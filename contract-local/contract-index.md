...
| 契約 | 用途 |
|---|---|
| [hmiIntf自訂角色尺度與美術規範](hmiIntf自訂角色尺度與美術規範.md) | 紙娃娃 rig、角色自然尺度、可玩角色 base／face layer 分層、臉部編輯器 UI 與正式美術資產規範（自 README ch2.7 拆出，issue #123 補入 base 分層，issue #130 改為固定底圖＋臉部編輯器） |
...
* **etyCfg**
  * `etyCfg通用兒童玩家`、`etyCfg通用家長維護者`、`etyCfg通用靜態主機平台`
...
* **runAct**
  ...
  * `runAct自訂玩家編輯公主臉部`：以臉部編輯器切換髮型、眉毛、眼睛、鼻子、嘴巴並調整膚色與髮（眉）色（issue #130）。
* **setAct**
...
* **datIntf（建議補建）**
  ...
  * `datIntf自訂玩家公主識別設定`：目前帳號的公主模板、名字、識別色、背景花紋與 face config（髮型／眉毛／眼睛／鼻子／嘴巴／膚色／髮色）之單一狀態來源，供公主選單、帳號卡、人物資訊欄與臉部編輯器共用（issue #130）。
