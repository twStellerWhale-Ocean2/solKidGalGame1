...
| 契約 | 用途 |
|---|---|
| [hmiIntf自訂角色尺度與美術規範](hmiIntf自訂角色尺度與美術規範.md) | 紙娃娃 rig、角色自然尺度、可玩角色 base 分層與正式美術資產規範（自 README ch2.7 拆出，issue #123 補入 base 分層） |
...
* **datIntf（建議補建）**
  ...
  * `datIntf自訂角色音色目錄`：角色特性維度（性別／年齡／性格…）相互組合之音色項與各項音頻參數（pitch／rate／lang／偏好 voice hint／fallback policy）對照之單一資料來源，含 `default` 降級項；角色（NPC 與可玩公主）以特性宣告對應一個音色項（issue #93、issue #109）。
