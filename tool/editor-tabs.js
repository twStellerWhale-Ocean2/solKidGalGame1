// 管理網站版面導覽（issue #260 / spec#13；依 [hmiIntf通用視覺規範] 管理網站規範）。
...
// 深連結沿用 panel 名（如 #voice）；無對應 panel 的資料包節點（未來才有頁的「遊戲規則」）不顯示。
const GROUPS = [
...
menuBtn?.addEventListener("click", () => shell.classList.toggle("collapsed"));
...
for (const g of GROUPS) {
  ...
  b.innerHTML = `<span class="nav-ico">${g.label.slice(0, sp)}</span><span class="nav-txt">${g.label.slice(sp + 1)}</span>`;
...
function selectPanel(name) {
  ...
  // Breadcrumb（管理工具 › 資料包）＋ Page Title（葉節點）。
  const leaf = group.panels.find((p) => p.panel === name)?.label || name;
  if (crumbEl) crumbEl.textContent = "管理工具 › " + group.label;
