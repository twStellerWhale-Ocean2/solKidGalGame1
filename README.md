...
- 2026-06-17（issue #131）：設計強化公主主題顏色應用。識別色色盤由 16 高飽和色改為 8 種低飽和粉彩色，並新增調色器供自訂任一色（既有存檔識別色相容保留、不被重置）；新增可選背景花紋（波浪／泡泡／格紋等 ≥8 種）與識別色組成公主視覺主題；選角卡、資訊欄與帳號卡的大頭照底色改用識別色半透明（alpha≈0.30）鋪底；地圖公主 token 由糊化光暈改為半透明識別色橢圓背版。本項為 2plan 初稿，待 dev／opr 校準。
