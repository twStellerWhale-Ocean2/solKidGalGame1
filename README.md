...
維護者的**管理設定工具**（衣物對位、場景對話、角色語音指定、新局起始等組態）只在**本機 dev 環境**提供入口，正式遊玩端（自架正式服務）**不顯示**這個入口；且部分設定的儲存需要本機伺服器寫回檔案，因此**必須在 localhost（或家庭 LAN）以 `node server.mjs` 執行**才能編修並儲存。詳見 [III.I 本地端管理模式（維護者）](#i-本地端管理模式維護者)。
