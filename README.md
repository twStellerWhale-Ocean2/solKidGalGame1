...
> 本節穩定播放與診斷已於 dev（issue #109）實作，實機聽感仍待 opr 終驗；依角色類型指定語音與首字前置留白為 issue #134 之 2plan 初稿，待 dev／opr 校準。
...
- 2026-06-18（issue #134）：設計改善角色語音。新增可於設定為各角色類型（依性別×性格）指定瀏覽器可用語音的「音色挑選層」（採同性別繼承式預設、可逐桶覆蓋），解決過去因 voice hint 比對失效而「不同人物聲音幾乎一樣」的問題；未指定時沿用語言優先 fallback（不硬綁具名 voice、維持跨平台可攜），年齡等差異續以音高／語速表現；並在送入 Web Speech API 前於文字開頭加入少量前置留白以改善首字清楚度。本項為 2plan 初稿，待 dev／opr 校準。
