...
2. 首次使用按「**建立新帳號**」：輸入帳號（小寫英文字母開頭、可含數字，3–16 字）與至少 6 位的密碼——年幼玩家可由家長協助輸入，密碼欄可切換「顯示密碼」避免打錯。這台裝置還沒有任何帳號卡時，登入畫面會直接呈現建立新帳號表單。若送出被擋（帳號格式不符、撞名、次數過多等），⚠ 開頭的錯誤說明會直接顯示在送出鈕**上方**、不會被手機鍵盤遮住，格式有誤的欄位也會同時標紅；送出過程中按鈕會顯示處理中（Creating…）——不會再有「按了沒反應」的情況。
