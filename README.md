...
- **場景互動三模組（生活聊天／逛店／打工）**：每個場景可依情境提供這三種互動、可只開部分——詳見下節〈場景互動：生活聊天、逛店、打工〉。
...
每個地點用同一套場景模板，可依情境**選擇性開啟**三種互動（不一定三種都有）：

- **生活聊天（Chat）**：和場景人物進行貼近生活的英文寒暄對話。答對會**提升心情**並在護眼上限內**延長這次的可玩時間**——用意是讓孩子感受到「和人互動是滿足自己的需求（心情變好、想多待一會兒）」，而不是為了換取別人的回饋。聊天不發 coins。
- **逛店（Shop）**：用 coins 購買外觀並換裝（沿用既有商店機制）。
- **打工（Part-Time Job）**：完成切合該場景的任務賺取 coins，題目可結合簡易數學或生活常識（例如算庫存、照順序上架）。打工所得即為勞動報酬。
...
- 場景互動分為生活聊天、逛店與打工三種、可選擇性開啟；聊天答對提升心情並在護眼上限內延長可玩時間，打工答對賺取 coins，回饋型別清楚分流。
