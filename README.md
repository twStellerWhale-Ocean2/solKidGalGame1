...
> ℹ️ 過渡說明：原 GitHub Pages 免安裝版已**凍結**於最後純靜態版本（v0.61.0，`pages-legacy` 分支），不再更新，將於後續增量（#311）正式下線；新功能一律走自架伺服器形態。

不做的事：不做 landing page、不做大型課程平台、不做密集 phonics 課程、不做後台商品管理、不做 email／第三方（Google 等）登入、不做公開多租戶營運服務（一家一伺服器、自架自管）。

## B. 快速開始

> 本節為 2plan 初稿，實際指令與 compose 檔由 code 段落地校準。

### (A) 部署（自架伺服器）

- 以 docker compose 啟動整組服務（node 服務＋PostgreSQL 資料庫），或等效方式自行啟動兩者；需提供 `DATABASE_URL` 與 `SESSION_SECRET` 兩個環境變數（`.env`）。
- node 服務**同站**提供遊戲網頁與 `/api/*` 帳號存檔端點，瀏覽器只需開一個網址；`/healthz` 可作服務健康檢查。
- 正式對外散佈（容器 image＋helm chart 整包）於增量 #311 提供；本階段以本機／家庭區網自架為主。
- `server.mjs` 仍是**維護者專用 dev 工具**（管理設定工具的寫回），與遊玩用服務無關。

### (B) 遊玩

1. 開啟你家自架伺服器的網址（部署者會告訴你，例如 `http://<主機IP>:4180/`）。
2. 首次使用按「**建立新帳號**」：輸入小寫英文帳號（3–16 字、字母開頭）與至少 6 位的密碼——年幼玩家可由家長協助輸入。
3. 之後每次進入，在登入畫面點自己的**帳號卡**（有公主大頭照與名字）、輸入密碼即可繼續冒險；同一帳號在手機、平板、電腦上進度一致。

### (C) 管理設定（需 localhost）

維護者的**管理設定工具**（衣物對位、場景對話、角色語音指定、新局起始等組態）只在**本機 dev 環境**提供入口，公開的 GitHub Pages **不顯示**這個入口；且部分設定的儲存需要本機伺服器寫回檔案，因此**必須在 localhost（或家庭 LAN）以 `node server.mjs` 執行**才能編修並儲存。詳見 [III.I 本地端管理模式（維護者）](#i-本地端管理模式維護者)。
...
- **登入帳號**：每次進入遊戲會先到登入畫面。這台裝置玩過的帳號會以**帳號卡**呈現——公主的頭胸部大頭照（以識別色半透明底色鋪底）、最近遊玩時間、coins 與目前可玩／休息狀態，點自己的卡、輸入密碼就進入；也可切換「其他帳號登入」或「建立新帳號」（小寫英文帳號＋至少 6 位密碼，家長可協助輸入）。不同帳號的 coins、日記、學到的字、擁有與穿搭、所在位置都各自獨立、互不混用；進度存在伺服器上，換一台裝置登入同帳號可以接著玩。剛從舊版升級的裝置，登入畫面會出現「匯入本機舊進度」，可把舊的本機帳號一鍵搬到伺服器帳號。（本段為 2plan 初稿，待 dev／opr 校準。）

  ![登入畫面設計參考稿（設計期參考稿·待 code 以實際截圖替換）](docs/design-visual/page-login.svg)
...
- **切換帳號與登出**：進入遊戲後可用返回初始選單按鈕回到登入／帳號選擇畫面，切換玩家（另一位玩家點自己的帳號卡輸入密碼）或調整公主設定；返回前會先把進度存到伺服器，不會重置進度，也不會解除尚未結束的休息鎖定。登出會撤銷這台裝置的登入狀態，下次需重新輸入密碼。玩家端沒有刪除帳號的功能（帳號管理屬維護者作業，於增量 #310 提供）。
...
- **角色語音指定改為維護者作業**（issue #246）：要把某類角色（依**性別與性格**）綁定到特定的瀏覽器語音（如 Windows 內建語音），由維護者在本機 dev 環境開啟**管理設定工具**的**聲音管理頁籤**設定；此設定為整台裝置共用（device-wide），公開的 GitHub Pages 網站不提供這個入口（見 [III.I 本地端管理模式](#i-本地端管理模式維護者)）。
...
> 本節為 2plan 初稿（issue #309 雲端存檔），行為細節待 dev／opr 校準。

- 進度（coins、穿搭、日記、位置、所選角色、名字與識別色）會以**目前登入帳號**為單位**自動存到伺服器**：一般變更稍作節流後寫入、重要時刻（答題結算、購買退款、換裝、時間到結算、登出）立即寫入；重整或換裝置登入同帳號皆還原到最後保存的進度。
...
> 本節說明「在本地端使用時會開啟的管理模式」。此模式**只在本機 dev 環境出現，公開的 GitHub Pages 不提供任何入口**。

當你把專案跑在**本機（localhost）或家庭 LAN** 的伺服器上時（`node server.mjs`），會多出一個**管理設定工具**（`tool/wardrobe-tuner.html`），供維護者集中調整各項內容組態；公開部署的 GitHub Pages 因為沒有可寫回檔案的伺服器，不會顯示這個入口。
...
- **儲存需本機伺服器**：衣物對位、旋轉、場景對話等編修按「儲存」後會**寫回 sidecar／資料檔**，因此需要本機或家庭 LAN 的 `node server.mjs`；公開 GitHub Pages 只能預覽、儲存會失敗並顯示提示。
...
- 2026-07-09（issue #309）：**方向轉換**——由「純靜態 GitHub Pages＋瀏覽器本機存檔」轉為「**自架伺服器＋帳號雲端存檔**」：玩家以小寫英文帳號＋至少 6 位密碼註冊登入（家長可協助），進度以帳號為單位存於伺服器（PostgreSQL）、跨裝置還原；遊戲端維持靜態無框架網頁（「靜態遊戲殼＋node API 核」），新增 node API 建置單元承接註冊、登入、session 與存檔；舊存檔可經 Markdown 匯入或登入畫面「匯入本機舊進度」一鍵遷移；玩家端不提供刪除帳號（帳號管理屬維護者，於 #310 提供）。GitHub Pages 免安裝版凍結於 v0.61.0（`pages-legacy`）、於 #311 隨 helm 整包發行正式退場。廢改 design.md spec#7／spec#8、新增 spec#23／spec#24。本項為 2plan 初稿，待 dev／opr 校準。
