...
- **經反向代理部署時**（啟用 Ingress 或 Cloudflare Tunnel 等邊緣入口）：依**實際代理跳數**設定環境變數 `TRUST_PROXY`（helm values `api.trustProxy`）——只有 Ingress 是 1、Tunnel 再接 Ingress 是 2（且 ingress-nginx 須開啟 forwarded headers），登入／註冊的失敗限流才會以真實來源計算。失敗限流一律**按「來源＋帳號」累計**（同一帳號重試才會被擋、鄰居的失敗不會鎖到你）；設定了 `TRUST_PROXY` 的公網部署，請以 Ingress 作為唯一對外入口、不要同時把 NodePort 暴露到公網（#331 增量設計中、待實作校準）。

### (B) 遊玩

1. 開啟你家自架伺服器的網址（部署者會告訴你，例如 `http://<主機IP>:4180/`）。
2. 首次使用按「**建立新帳號**」：輸入帳號（小寫英文字母開頭、可含數字，3–16 字）與至少 6 位的密碼——年幼玩家可由家長協助輸入，密碼欄可切換「顯示密碼」避免打錯。這台裝置還沒有任何帳號卡時，登入畫面會直接呈現建立新帳號表單。若送出被擋（帳號格式不符、撞名、次數過多等），錯誤說明會直接顯示在送出鈕**上方**、不會被手機鍵盤遮住；按下按鈕像「沒反應」時，請先看送出鈕上方的紅字說明（#331 增量設計中、待實作校準）。
