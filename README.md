...
本專案是一個給年幼英文學習者玩的靜態網頁遊戲。主要體驗不是一般網站、題庫或後台，而是手機直向優先的兒童向日式 ADV：孩子陪 Princess Lumi 去不同地點和角色對話，練習一個短英文句子，答對後拿 coins，再把 coins 立刻轉成可見的髮型、換裝、鞋子、帽子、配件或 outfit set 獎勵。房間佈置不屬於本輪正式商品 runtime，未來若恢復應走獨立 `decorate` flow。
...
- `wardrobe` 用於更換 Lumi 身上的裝扮，可透過 category 呈現 `Hair`、`Tops`、`Bottoms`、`Dresses`、`Outerwear`、`Shoes`、`Hats`、`Accessories`、`Outfit Sets`；`Outfit Sets` 是 bundle，不是新的紙娃娃 slot；`help` 代表幫助場景人物並取得回報；`shop` 用於購買獎勵；`refund` 只在原購買商店退還商品。
...
Forest 是 Kingdom 左側山林入口細化出的寬地圖，入口視覺需與 Kingdom 左側山、中段森林、右側河流的全景關係對照。Forest 場景以奇幻童話角色為主，目前包含 Elf Glade、Dwarf Cottage、Fairy Atelier、Stone Golem Pass、Halfling Village、Wizard Hut、Red Hood Path、Three Pigs Cottage、Tree Spirit Grove 與右側回 Kingdom 的 portal。Forest map 是純地點地圖，不在 map plate 上畫小紅帽、三隻小豬、精靈、矮人或樹靈；人物只在 scene NPC portrait 顯示。Forest 的正式 runtime 必須使用手繪風格 PNG / WebP 地圖、場景、NPC 與商品素材；臨時 SVG 或 CSS 幾何素材只能留作歷史來源或設計草稿，不得出現在正式遊玩引用中。
...
- 房間進入後先顯示功能 action choices：`Hair`、`Tops`、`Bottoms`、`Dresses`、`Outerwear`、`Shoes`、`Hats`、`Accessories`、`Outfit Sets`、`Leave`。
...
- Kingdom 地點包含：Luminara Castle、Castle Garden、School Classroom、Library、Temple、Administration Hall、Market Square / Bakery、Harbor Port、Fish Shop、Hair Salon、Dress Boutique、Tailor Studio、Shoe Shop、Accessory Atelier、Suburb Road、Forest Edge、Lighthouse。
...
- 正式商品分類為 `hair`、`tops`、`bottoms`、`dresses`、`outerwear`、`shoes`、`hats`、`accessories`、`outfitSets`。`outfitSets` 是購買後解鎖並套用既有 slot 商品的 bundle，不是新的紙娃娃 slot。
- 每個商店最多兩種商品分類，且 `storeId` 必須對應真實地點。Kingdom 城區：`Hair Salon` 賣 `hair`；`Dress Boutique` 賣 `dresses` + `outfitSets`；`Tailor Studio` 賣 `tops` + `bottoms`；`Shoe Shop` 賣 `shoes`；`Accessory Atelier` 賣 `hats` + `accessories`；`Market Square` 是 help / food 市集場景，不販售家具或 wardrobe 商品。
- 城區外也可以買賣東西，但仍遵守地點語意與每店最多兩類：Castle 的 `Royal Cloak Room` 賣 `outerwear` + `hats`、`Castle Seamstress` 賣 `tops` + `bottoms`；Suburb 的 `Workwear Stall` 賣 `tops` + `bottoms`、`Field Cobbler` 賣 `shoes` + `hats`；Forest 的 `Dwarf Cottage` 賣 `outerwear` + `shoes`、`Fairy Atelier` 賣 `dresses` + `accessories`。
...
- Shop 採購清單只顯示尚未擁有的商品；已購買的 wearable 商品從 Shop 清單消失，後續只能在 Wardrobe 管理與更換。披風、斗篷與外套歸 `outerwear` / `outer` slot，不歸 accessory。`room` / 家具商品不屬於正式 runtime，未來若恢復房間佈置，另開 `decorate` flow。
...
- 每個商店與地點要有自己的背景與人物：例如 Bakery / Market、Hair Salon、Dress Boutique、Tailor Studio、Shoe Shop、Accessory Atelier、Royal Cloak Room、Dwarf Cottage、Fairy Atelier、Garden、Harbor、Farm、Lighthouse 不能只換文字。
...
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-scene&place=hairSalon#map`
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-scene&place=tailorStudio#map`
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-scene&place=royalCloakRoom#map`
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-scene&place=fairyAtelier#map`
...
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-detail&place=boutique&item=roseClassicSet#map`
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=shop-not-enough&place=boutique&item=roseClassicSet#map`
...
- `http://127.0.0.1:4174/?selftest=visual-qa&surface=forest-map&place=forestEntrance#map`
...
- Princess Room 進入後先顯示 `Hair`、`Tops`、`Bottoms`、`Dresses`、`Outerwear`、`Shoes`、`Hats`、`Accessories`、`Outfit Sets`、`Leave` 等 action choices；選擇後才開 Wardrobe detail panel。
- Shop 進入後先顯示 `Help`、`Shop`、`Refund`、`Leave` 等 action choices；選 `Shop` 或 `Refund` 後才顯示對應 detail panel。
- Princess Room 第一層不再顯示 `Room Treasures`；`room` / 家具商品不屬於本輪正式 runtime，未來以獨立 `decorate` action type 回歸。
...
- Split shop categories by place:
  - Hair Salon: hair
  - Dress Boutique: dresses / outfit sets
  - Tailor Studio: tops / bottoms
  - Shoe Shop: shoes
  - Accessory Atelier: hats / accessories
  - Royal Cloak Room: outerwear / hats
  - Castle Seamstress: tops / bottoms
  - Workwear Stall: tops / bottoms
  - Field Cobbler: shoes / hats
  - Dwarf Cottage: outerwear / shoes
  - Fairy Atelier: dresses / accessories
  - Market: help / food scene only, no furniture shop
