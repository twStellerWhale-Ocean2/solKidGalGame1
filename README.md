...
- **備份你的資料庫**：全家的帳號與進度都存在 PostgreSQL 裡，請定期備份（`pg_dump` 級指令，實際指令待 code 段校準寫回本節）；玩家也可各自在遊戲內匯出 Markdown 備份。
- **重設密碼（過渡）**：孩子忘記密碼時，由維護者在伺服器端以維護指令重設（指令待 code 段校準寫回本節）。
- `server.mjs` 仍是**維護者專用 dev 工具**（管理設定工具的寫回），與遊玩用服務無關。

### (B) 遊玩

1. 開啟你家自架伺服器的網址（部署者會告訴你，例如 `http://<主機IP>:4180/`）。
2. 首次使用按「**建立新帳號**」：輸入帳號（小寫英文字母開頭、可含數字，3–16 字）與至少 6 位的密碼——年幼玩家可由家長協助輸入，密碼欄可切換「顯示密碼」避免打錯。
3. 之後每次進入，在登入畫面點自己的**帳號卡**（有公主大頭照與名字）、輸入密碼即可繼續冒險；同一帳號在手機、平板、電腦上進度一致。
4. 忘記密碼時，由部署的家長／維護者在伺服器端以維護指令重設（過渡做法；線上管理於後續增量 #310 提供）。

### (C) 管理設定（需 localhost）

維護者的**管理設定工具**（衣物對位、場景對話、角色語音指定、新局起始等組態）只在**本機 dev 環境**提供入口，正式遊玩端（自架正式服務；含凍結之 `pages-legacy` 靜態頁）**不顯示**這個入口；且部分設定的儲存需要本機伺服器寫回檔案，因此**必須在 localhost（或家庭 LAN）以 `node server.mjs` 執行**才能編修並儲存。詳見 [III.I 本地端管理模式（維護者）](#i-本地端管理模式維護者)。
...
> 本節說明「在本地端使用時會開啟的管理模式」。此模式**只在本機 dev 環境出現，正式遊玩端（自架正式服務；含凍結之 `pages-legacy` 靜態頁）不提供任何入口**。

當你把專案跑在**本機（localhost）或家庭 LAN** 的伺服器上時（`node server.mjs`），會多出一個**管理設定工具**（`tool/wardrobe-tuner.html`），供維護者集中調整各項內容組態；正式遊玩端（自架正式服務不含此寫回端點；凍結之 `pages-legacy` 亦無伺服器）不會顯示這個入口。
...
- **儲存需本機伺服器**：衣物對位、旋轉、場景對話等編修按「儲存」後會**寫回 sidecar／資料檔**，因此需要本機或家庭 LAN 的 `node server.mjs`；正式遊玩端（自架正式服務與 `pages-legacy`）只能預覽、儲存會失敗並顯示提示。
