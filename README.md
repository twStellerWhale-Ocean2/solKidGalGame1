...
- 聲音以瀏覽器內建 Web Speech API 呈現；系統會等待瀏覽器 voice 清單載入，優先依語言與角色偏好選 voice，缺對應設定時自動退回預設嗓音，不會出錯或卡住。
- 為了讓兒童更容易聽清楚，語音會以**較慢的速度**朗讀（基準約為原本速度的 80%），英文與中文協助都一樣放慢；各角色之間的相對快慢仍會保留。
- 為避免中英文按鈕第一個字被吃掉，語音播放會以集中式 speech manager 管理佇列、重播與停止，不會在每次播放前無條件取消既有語音。
...
- 不同場景人物與玩家公主各有貼合角色的聲音，公主會唸出所選的答案；中英文語音首字清楚、語速約 80%，缺合適瀏覽器 voice 時會降級並記錄；關閉 Voice 後語音靜音、遊戲仍可玩。
