...
2. 未登入時看到的是**登入表單**（帳號＋密碼；帳號存在伺服器上——換一台裝置的既有玩家直接登入即可，進度會跟著回來、不必也不該重建帳號）。第一次使用的新玩家點表單下方的「First time here? Create an account」建立**家庭帳號**：輸入帳號（小寫英文與數字 3–16 字，**可用數字開頭**、至少含一個英文字母）與至少 8 位、**含一個數字與一個小寫英文**的密碼——年幼玩家可由家長協助輸入，密碼欄可切換「顯示密碼」避免打錯。若送出被擋（帳號格式不符、撞名、次數過多等），⚠ 開頭的錯誤說明會直接顯示在送出鈕**上方**、不會被手機鍵盤遮住，格式有誤的欄位也會同時標紅；送出過程中按鈕會顯示處理中（Creating…）——不會再有「按了沒反應」的情況。

   ![首次進入：登入畫面（新裝置預設；註冊為下方次要連結）](docs/manual-assets/issue357-01-signin-default.png)

   ![點「First time here? Create an account」後的建立新帳號表單](docs/manual-assets/issue357-02-register-secondary.png)

   ![送出被擋時：⚠ 錯誤顯示在按鈕上方、欄位標紅（手機視口）](docs/manual-assets/issue331-01-register-error-mobile.png)

3. **登入一次就保持登入**：之後每次開啟遊戲不必再輸入帳密，直接到「**Who is playing?**」**選角色頁**——上方顯示帳號名稱與遊玩狀態，中間是這個帳號的所有公主（大頭照＋名字＋coins），**點自己的公主就開始玩**；下方「Add princess」可為家裡另一位小朋友（或同一位想多養一位）新增公主，每位公主的 coins、日記、學到的字與穿搭**各自獨立**。同一帳號在手機、平板、電腦上進度一致。

   ![登入後的選角色頁（Who is playing?）](docs/manual-assets/issue390-07-character-home-resume.png)

4. **公主小密碼（選設）**：新增公主時可以幫她設一個小密碼（留空＝不設）；設了密碼的公主要輸入正確密碼才能進入或刪除，避免家裡小朋友互玩對方的進度。每位公主旁還有 ⓘ（看名字、coins、衣櫃、學到的字等資訊）與 ×（刪除——沒設密碼的按兩段確認；最後一位公主不能刪）。**登出在選角色頁上方**：登出後才需要重新輸入帳密（帳號欄會自動預填上次的帳號）。
5. 忘記**帳號**密碼時，請部署的家長／維護者在**線上管理頁**（`/admin/`）幫你重設新密碼（見 [III.J 線上管理](#j-線上管理維護者)），再用新密碼登入即可，進度不受影響；忘記**公主**小密碼時，可用存檔備份匯出再匯入的方式由家長協助處理。
6. **舊版多帳號家庭想合併成一個帳號**（例如每個小孩原本各有一個帳號）：先登入舊帳號、在遊戲內「Save」匯出該公主的存檔備份（.md 檔），再登入要保留的家庭帳號、「Load」匯入並選「**加入現有公主**」——這位公主就會帶著完整進度加入這個帳號的公主清單。
7. 若登入表單下方沒有「Create an account」而是顯示「This server is not taking new accounts right now. Please ask your grown-up to open it.」（本伺服器目前不開放新帳號，請找家長開放），代表維護者已關閉註冊——請找部署的家長／維護者開放後再註冊。
...
- 多人（同裝置或不同裝置）可用各自帳號保留獨立進度；帳號登入一次即保持登入、每次上線經「選角色頁」選公主進入，切換公主與登出（皆於選角色頁）順暢；帳號密碼以業界標準雜湊保存、登入失敗提示不洩漏帳號存在性。
...
```text
登入帳號（帳密登入一次即保持登入；首次先「Create an account」）
  → 選角色頁（Who is playing?）：點自己的公主進入；可新增／檢視／刪除公主、登出
  → 選角＋命名＋設定視覺主題：新公主隨機識別色＋背景花紋，可改粉彩色盤／調色器自訂／改花紋
  → Princess Room → Castle Map → World Map → Area Map
  → 進入地點 Scene → 選動作 → 生活聊天 / 打工 / 逛店 / 換裝
  → 回饋（心情 / coins / 日記 / 學到的字 / 換裝）→ 自動存到伺服器 → 再來一輪
```

- **登入帳號與選公主（兩張表單）**：未登入時先到**登入表單**（帳號＋密碼；小寫英數帳號＋至少 8 位、含一個數字與一個小寫英文的密碼，家長可協助輸入），**登入一次後這台裝置保持登入**。之後每次開啟遊戲直接到「**Who is playing?**」**選角色頁**——上方是帳號名稱、公主數與遊玩狀態（含 Log out），中間列出這個帳號的所有公主（大頭照＋名字＋coins，點列即進入；設了小密碼的公主要先輸入密碼），下方「Add princess」新增公主。每位公主的 coins、日記、學到的字、擁有與穿搭、所在位置都各自獨立、互不混用；進度存在伺服器上，換一台裝置登入同帳號可以接著玩。剛從舊版升級的裝置，登入表單下會出現「Import old local progress」，可把舊的本機帳號一鍵搬到伺服器帳號（若目標帳號已有雲端進度會先警示確認才覆蓋）。輸入錯誤密碼會就地顯示「Username or password is incorrect」、不洩漏帳號是否存在。

  ![登入後的選角色頁（Who is playing?）](docs/manual-assets/issue390-06-character-home.png)
...
- **選角與命名**：首次進入會先選公主外觀、輸入名字並設定視覺主題——新帳號會先取得一次性隨機識別色與背景花紋（如波浪、泡泡、格紋等），玩家可改從飽和度較低的粉彩色盤選擇、用調色器自訂任一色，或改選背景花紋；之後可再叫出此畫面重選外觀、改名、改色或改花紋（不影響既有進度）。可玩公主 roster 為 Lumi、Yumi、Rosa 三位；既有帶 `sol`（舊名 Mary）角色 id 的舊存檔於讀取時自動 fallback 為預設角色 `lumi`，可無縫升級至三角色 roster。選角色頁與遊戲內人物資訊欄的大頭照卡片皆以該識別色的半透明底色鋪底。
...
- **切換公主與登出**：遊戲畫面上方的 ⟳（Switch princess）會回到**選角色頁**——換另一位公主玩、新增／檢視／刪除公主、或按 Log out 登出，全部都在這一頁；返回前會先把進度存到伺服器，不會重置進度，也不會解除尚未結束的休息鎖定（護眼時鐘綁帳號、跨公主共用，換公主不能繞過休息）。登出會撤銷這台裝置的登入狀態，下次需重新輸入密碼（帳號欄會預填）。玩家端沒有刪除帳號的功能（帳號管理屬維護者作業，見 [III.J 線上管理](#j-線上管理維護者)）。
...
當你把專案跑在**本機（localhost）或家庭 LAN** 的伺服器上時（`node server.mjs`），會多出一個**管理設定工具**（`devtool/wardrobe-tuner.html`），供維護者集中調整各項內容組態；正式遊玩端（自架正式服務不含此寫回端點）不會顯示這個入口。

- **開啟方式（一鍵）**：檔案總管雙擊 **`devtool/start-devtool.cmd`**（或終端機 `pwsh devtool/start-devtool.ps1`，支援 `-Port`／`-NoOpen`）——自動起本機 server（已在跑則直接複用）並開啟工具頁。手動備路：`node server.mjs` 後自行開 `http://127.0.0.1:<port>/devtool/wardrobe-tuner.html`。`server.mjs` 預設監聽 `0.0.0.0`，啟動 log 會顯示 LAN IP，**同一家庭區網內的手機或平板可直接以該 IP 開啟**管理設定工具。若需指定監聽位址，可設 `HOST` 環境變數覆寫（例：`HOST=127.0.0.1 node server.mjs`）。
...
- **孩子忘記密碼**：點該帳號的「重設密碼」、輸入新密碼即可（規則同註冊：至少 8 位、含一個數字與一個小寫英文；**孩子既有的舊密碼不受新規則影響、照常登入**）；重設後孩子所有裝置需用新密碼重新登入。改**自己（admin）**的密碼也是同一顆按鈕——目前這台不會被登出，其他裝置要用新密碼重登。
