...
- **選角與命名**：首次進入會先選公主外觀並輸入名字；之後可再叫出此畫面重選外觀或改名（不影響既有進度）。可玩公主 roster 規劃為 Lumi、Yumi、Sol、Rosa 四位，既有 Lumi／Yumi／Sol 存檔 id 保持相容。
...
> 本項為 2plan 初稿，角色素材與 manifest 實作待 dev／opr 校準。
...
紙娃娃 base 必須是換裝中性底，不固定長髮、長袖或睡衣；預設髮型與預設衣物也應由可替換 layer 呈現。這樣短髮、短袖與露手臂衣物才不會露出舊長髮或舊袖子。
...
- 2026-06-15（issue #123）：設計重整可玩公主 base 分層與四角色 roster。可玩公主 base 改以換裝中性底為目標，不再烘入長髮、長袖或睡衣；預設髮型與預設衣物改由可控制 layer 呈現；Lumi 保留方向、Yumi 重製為冷色優雅系、Sol 重製為陽光短髮系，新增棕髮甜美系 Princess Rosa（id `rosa`），並保留既有 `lumi`／`yumi`／`sol` 舊存檔相容。本項為 2plan 初稿，待 dev／opr 校準。
