...
* 桌機**向內容較多的購物對齊**（避免壓縮商品格／試穿預覽；場景面板一致地變高、接受其下方留白），與 #111「外框向 shop 近全高對齊」同philosophy。
* 確切值/響應式公式（固定 px、`dvh` 式、或佔外框比例）留 **3code visual-QA** 定案；建議採類 `--adv-scene-height` 之 `min(px, dvh 式)` 響應式，桌機使 scene/shop 同高且 ≥ 購物內容所需。手機現已 316 一致，宜折入同一來源公式（值可不同、機制統一）。
...
## 4. 審查點（待 USR 於 Draft PR 確認）

* **統一方向**：採 D2「桌機向購物對齊」（場景面板變高），或改採折衷值/向場景對齊（須確認購物商品格容性）。
* **一致性層級**：是否僅收斂 `.adv-box` 外緣高度，或一併規範內層（商品格捲動區、立繪舞台）之容性。
