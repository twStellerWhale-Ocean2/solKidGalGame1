...
### D4：卡片半透明底色（收斂三處為單一作法）

* 選角卡 `.character-portrait`、資訊欄 `.info-portrait`、帳號卡 `.account-avatar` 三處統一改為「識別色半透明」鋪底（`color-mix(in srgb, var(--active-profile-color) <alpha>, transparent)`），取代現行「純色鋪滿」與帳號卡的 `color-mix 18%, #fff`——**收斂為單一柔化來源**，不再三處各自為政（呼應「別疊床架屋」）。
* **alpha 校準**：USR 原指定 alpha=0.30；實機 visual-qa（行動直向）發現 bust（粉色 playwear）佔據卡面，識別色僅邊緣可見、0.30 於小頭像辨識偏弱（spec#8）。經 USR「無偏好／採推薦」同意，校準為 **0.45**（仍半透明、辨識更佳）。design.md `paramCardBackgroundAlpha=0.45`。
* 頭胸 bust 疊於底版之上，半透明底不影響頭像與文字辨識。
...
* `tsc --noEmit --project jsconfig.json` exit 0；`repoLint` 0 違規；`docLint`(sol) 0 違規。
* 瀏覽器 selftest（headless chromium、行動直向 412×880）全 passed、console 0：**profile-color**（擴充 intTest#31/#40：色盤＝8 粉彩、調色器自訂色 `#123abc` 保留、舊 16 色值 `#ef4444` 相容不重置、非法色回退角色預設、背景花紋集 ≥9（none+8）與 normalize、資訊欄卡 `data-pattern` 套用）、save-load、accounts、map-avatar、data-audit、chat。
* 依賴安全：純靜態站、無 package.json／runtime 依賴 → 無 `npm audit` 標的（豁免理由：無依賴清單）。

### §5 業界水準審查（鏡頭 A/B/C，行動直向 412×880；比照 #132 輕量記錄）

異動頁逐頁實機 visual-qa（surface=`character-select`／`world-map`／`account-select`／`map-near`）＋ DOM 斷言佐證。重點發現與分級：

* [務必要修 → 已修] **卡片識別色 alpha 0.30 辨識偏弱**：bust（粉色 playwear）佔卡面、識別色僅邊緣可見，0.30 於小頭像難辨多帳號（spec#8）。→ 校準為 **0.45**；Yumi(藍)／Rosa(綠)／Sol(琥珀) 等識別色明顯可辨（見 D4）。
* [可接受] 8 粉彩色盤一列、彼此可辨；調色器 Custom 入口與選中 ring 清楚；選色即時反映於選角卡。
* [可接受] 背景花紋選擇器（— ＋8）一列、選中 ring；花紋為白色半透明圖樣疊於識別色底、bust 之下，不損頭像／文字可讀性。
* [可接受] 地圖公主 token：DOM 確認舊 `drop-shadow` 光暈已移除、改 `.map-doll::before` 半透明識別色橢圓背版（`--profile-color` 套用），邊緣不再糊化。
* [可接受] 資訊欄識別卡：DOM 確認 `data-pattern` 與識別色 @ 0.45 正確套用；時間／coins／名稱版型不受影響。
* [可接受] 帳號卡識別底色收斂為同一作法、與選角／資訊欄一致（跨畫面一致性）。
* [後續辦理] 花紋為色彩無關之白色圖樣（非識別色染色）——刻意取捨以確保任一識別色上皆可見；未來可評估識別色染色花紋。
* 鏡頭 A（HMI 能力）：選色／選花紋皆有即時回饋、選中態與預設值；無空白／錯誤態回歸。鏡頭 B（缺口）：舊存檔相容（自訂色／舊色值不重置）、未知花紋回退 none，已由 intTest#40 覆蓋。

### 「別疊床架屋」收尾檢核

* 橢圓背版上線後**已移除** #126 的 `.map-doll .paper-doll-layer` 識別色 `drop-shadow` 光暈（僅留深色投影＋白描邊維持清晰）。
* 舊 16 色色盤移除後，`normalizeProfileColor` 改為格式驗證、無白名單殘留；profile-color selftest 已改驗 8 色，無硬編碼舊色盤殘留引用。
