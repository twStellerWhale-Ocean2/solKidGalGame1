...
## 6. 實作與驗證結果（3code，2026-06-17）

> 沿 #101/#111/#120：本焦點 UI 修正不另產 `docs/test-summary.html`，GATE 驗證結果記於本節。

### 實作

* `index.html`：`.side-panel` 重構為 `.info-bar`（大頭照 bust `data-doll="side-bust"` ＋ 四欄位 Player/Coins/Play time/Time left ＋ 兩 `.info-icon-button` ⚙/⟳）；移除 subtitle、status-grid（含 Mood）、energy-meter、equipped-box；`#statusMessage` 改為 `visually-hidden` aria-live（保留即時回饋給輔助科技、不再佔可見框）；`#princessNameTitle` 改為 visually-hidden h1（文件結構）。
* `game-engine/system/play-clock.js`：新增 `playAllowance(state)`（由時戳推導 base/bonus/total 分鐘，不新增 state 欄位）。
* `game-engine/main.js`：HUD 改為 `renderPlayTimeAllowance()`（`15` / `15 +3😄 min`）＋剩餘倒數；`updateProfileColorChrome` 不再以背景圖渲染側欄頭像、改同步識別色（bust 由 `renderPaperDolls` 填層）；`renderIdentity` 去「Princess」綴字；`renderStatus` 移除 mood/outfitSummary 綁定。
* **單一頭胸渲染機制（sysCase#5.2「不另維護第二套裁切」）**：新增 `bustMarkupFor`＋`renderBustInto`，**側欄、帳號卡與選角卡全部改用同一個 bust**（同一紙娃娃層合成＋同一 `.bust-doll` 裁切）；**移除**舊的 `applyProfileAvatar`／`profileAvatarStyle` 背景圖裁切（第二套邏輯）。選角卡 `.character-portrait` 退為純外框、以空 outfit 渲染各候選公主基本造型。
* `game-engine/app/elements.js`：新增 `playerNameValue/playTimeValue/timeLeftValue`，移除 `moodValue/energyValue/energyMeterFill/outfitSummary`。
* `styles/base.css`（寬版）／`styles/mobile.css`（窄版三欄 HUD）／`styles/paper-doll.css`（`.bust-doll` 頭胸裁切；採與選角卡／公主選單同一組參數 `background-size:245%`、`position:50% 47%`，三處框法一致）／`styles/account-select.css`（`.account-avatar` 改方形 bust 框）。
* `game-engine/testing/selftests.js`：intTest#31 側欄頭像檢查由「背景圖」改為「bust 紙娃娃層數」。

### GATE §1（機器判定，全數通過）

* `npx -p typescript tsc --noEmit --project jsconfig.json` → exit 0
* `docLint docs/design.md`（sol）→ PASS（0）；`repoLint .` → PASS（0）
* 回歸 selftest（headless Playwright）全 `passed:true`、console 0 error：`save-load`／`monkey`／`mood-extend`／`profile-color`（含 intTest#31）／`data-audit`／`accounts`／`map-avatar`／`chat`／`playtimer`／`about`。

### GATE §5（實機 visual-qa，寬版＋窄版）

| 版型 | 視窗 | 結果 |
|---|---|---|
| 寬版（桌機側欄） | 1440×900 / 1280×900 | 大頭照＋⚙/⟳ 上排、Player/Coins/Play time/Time left 直向四欄，無 overflow，內容對齊頂部 |
| 窄版（行動 HUD） | 390×844 | 三欄：左 bust 大頭照｜中 2×2 欄位｜右 ⚙/⟳ 直放，浮於場景上、無 overflow |

* 大頭照即時穿搭：未換裝顯示 baked-in playwear 頭胸；equip（peachSailorTop/pearlTiara/starCape）後 bust 即反映 4 層穿搭（tiara/top/cape 可見）。
* 帳號卡：avatar 改為方形 bust（各帳號自己的穿搭），取代舊圓形底圖。
* 時間顯示：起始 `15 min`／剩餘 `15:00`；生活聊天延長後 `15 +3😄 min`（`+3` 帶 `.play-time-bonus` 綠色、😄 緊接）／剩餘 `18:00`。
* 已移除元素（Mood 數字、energy 量表、Equipped 框、提示框可見性、Princess 綴字、subtitle）皆不殘留；`#statusMessage` 改 sr-only 仍即時更新（不破壞回饋）。
