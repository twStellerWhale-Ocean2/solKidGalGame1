...
* `game-engine/main.js`：HUD 改為 `renderPlayTimeAllowance()`（`15` / `15 +3😄 min`）＋剩餘倒數；`updateProfileColorChrome` 不再以背景圖渲染側欄頭像、改同步識別色（bust 由 `renderPaperDolls` 填層）；`renderIdentity` 去「Princess」綴字；`renderStatus` 移除 mood/outfitSummary 綁定；新增 `bustMarkupFor`，帳號卡改用各帳號穿搭渲染 bust。
