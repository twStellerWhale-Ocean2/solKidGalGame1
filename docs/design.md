...
* **建置指令**：遊戲殼無打包（靜態收集）；modApi `cd sysApi && npm ci && npm run build`（TypeScript→`dist/`）；正式 image 根目錄 Dockerfile 多階段 `docker build -t ghcr.io/twstellerwhale-ocean2/sollingoworld:<VERSION> .`（非 root、COPY 對齊靜態 allowlist、`devtool/`／測試不入包、`linux/amd64`）；chart `helm package deploy/helm`（chart `version`／`appVersion` 與 `VERSION` 同源）。
