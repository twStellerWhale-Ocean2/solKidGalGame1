...
* **[paramAssetDelivery]**（靜態資產傳輸層，issue #362）：`paramImageCacheControl=public, max-age=600, stale-while-revalidate=86400`——**僅圖像副檔名**（webp/png/jpg/gif/svg/ico）適用；`content-package` 內之 JS（manifest／rules／index.generated）與 `game-engine`／`styles`／`index.html` 一律維持 revalidate（升級即生效，防新引擎配舊資料／舊殼）。`stale-while-revalidate` 使過期後仍先以快取即時繪、背景更新——畫面永不阻塞等網路，升級後最多一次舊圖。`paramScenePrefetch=proximity-based`：玩家**走近／聚焦**某地點時預抓其場景圖與 NPC 圖（四區一致：城堡與 travel 地圖之對稱路徑皆掛；互動模型「首點聚焦、次點進場」提供天然窗口，走動亦持續觸發）——故一場遊玩下來實質會抓到沿途經過之地點，非只有最終進入者；**不做整區一次抓**（全場景圖 15.9 MB）。預抓一律 `fetchPriority="low"`（不得與玩家當下正在看的圖搶頻寬）、fire-and-forget（失敗不阻斷進場）、模組級 Set 去重（走動時每步觸發、僅首次建 Image）。
