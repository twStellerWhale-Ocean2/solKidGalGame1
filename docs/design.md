...
* **spec#19-可玩角色 roster 精簡為三位（Lumi、Yumi、Rosa）**：可玩公主**外觀類型** roster 為三位可辨識公主（每帳號可存多位使用此類型之角色，見 spec#28）；既有帶已移除角色 id 之舊存檔於讀取時 fallback 為預設角色而不 crash、不殘留。
...
* **[paramAssetDelivery]**（靜態資產傳輸層，issue #362）：`paramImageCacheControl=public, max-age=604800, stale-while-revalidate=604800`（7 天）——**僅圖像副檔名**（webp/png/jpg/gif/svg/ico）適用；`content-package` 內之 JS（manifest／rules／index.generated）與 `game-engine`／`styles`／`index.html` 一律維持 revalidate（升級即生效，防新引擎配舊資料／舊殼）。長新鮮期之安全前提＝**圖像 URL 全數帶版號**（`?v=…`，換圖必改版號＝新 URL⇒零陳舊）；此前提目前為人工紀律、尚無機判（次批補 assetLint 版號守門——在此之前忘記 bump 版號之失誤將最長陳舊 14 天且無 flush 手段）。效益落點＝**跨頁載入**（新 document：每張圖省一趟條件請求 RTT）；同分頁內重進場景本由瀏覽器記憶體快取接住、與本參數無關（issue #362 實測證實）。`paramScenePrefetch=focus-triggered`：玩家**聚焦**某地點（點該地點標記＝互動模型「首點聚焦、次點進場」之第一點）時預抓其場景圖與 NPC 圖，四區一致（城堡 `focusCastleHotspot`／travel `focusTravelHotspot` 皆掛）——此為**實測證實**之路徑（Fast-3G：省下約 4 秒乾等）。鄰近鉤子（`updateNearbyCastleHotspot`／`updateNearbyHotspot`）亦掛有預抓，惟**實測走動未觸發**（探針：玩家位移後 nearby 標記數仍 0、零預抓），故不宣稱「走動即抓」——其成因與是否值得修列次批（issue #362 ＜IV＞）。**不做整區一次抓**（全場景圖 15.9 MB）。預抓一律 `fetchPriority="low"`（不得與玩家當下正在看的圖搶頻寬）、fire-and-forget（失敗不阻斷進場）、模組級 Set 去重（走動時每步觸發、僅首次建 Image）。
