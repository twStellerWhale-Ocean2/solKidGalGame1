...
* **sysStory#5-承接選角與內容擴充**：
  ...
  * **sysCase#5.3**：[modContent模組]承接[setAct自訂維護者擴充內容]，匯入新內容包至 registry；可玩公主與 wardrobe layer 均須遵守 [hmiIntf自訂角色尺度與美術規範] 的 `shared-512x768-v1` rig；可玩公主 base、wardrobe layer 與 ADV 場景背景須以 GPT 產生或修圖為童話手繪風格 transparent/raster 素材，不得以 SVG、CSS 濾鏡、向量拼貼、模糊補版或 renderer 特例代替；新增 wardrobe item 須依 `type`／slot 繼承類別級 layer bounds、且每件至多對應單一 layer slot（單品單層，不採 bundle 套裝或單件多層）；商店商品預覽直接重用該商品 wardrobe layer 素材（單一素材，不另設分離商品縮圖）；新增或替換場景背景須交付完整繪製之 `1024x1024` WebP，並在 manifest 以 `sceneArt.src` 指向正式資產。
...
* 預期結果：
  ...
  4. 商店商品預覽與投影同源：每件 wardrobe item 之預覽 `image` 即其 `layers[0].src`（單一素材，無分離商品縮圖殘留）。
...
* 預期結果：
  ...
  2. 每題以 paramJobChoiceCount（3）個選項呈現；題幹為場景角色以第一人稱向公主提出之具體工作請求（角色實際需公主代勞、切合場景主體之勞務差事，排除純觀看、站位、寒暄或道別等非勞動內容）、選項為公主應允並完成該工作之回應或任務回報（非 Pick／Tell 之 meta 指令），題組歡迎詞與干擾選項切合場景主體、不另設題組開場白旁白、無超現實或指涉英文字之 meta 敘述；且正解以自然應允語句開頭（合該地區英文分級，如「Sure thing」「OK, I can …」「Well, I think I can help …」），可由 selftest data-audit 以允收開頭清單與打工題工作性（題幹須為勞務差事、非純觀看／站位／寒暄／道別）核對。
...
* productReadme 要求：
  1. 說明以 GitHub Pages 部署、擴充內容包與移除部署的方式，並指向可玩公主 `shared-512x768-v1` rig 與 base／wardrobe layer 分層契約；維護者須知道角色 base、wardrobe layer 與場景背景應使用 GPT 產生或修圖之童話手繪風格 raster 素材，不可用 SVG、CSS 濾鏡、模糊補版或 renderer 特例代替，且商店商品預覽直接重用 wardrobe layer 素材、不另設商品縮圖，新增衣物須繼承類別級 layer bounds，新增或替換場景背景須為完整繪製之 `1024x1024` WebP。
* 通過判定：
...
* **spec#3-可把學習成果轉為看得見的外觀獎勵**
  ...
  * 觀察項目：購買件數、換裝次數、coins 留存、base 對位錯誤率、wardrobe 類別級 layer bounds 覆蓋率、代表性衣物實穿對位 QA 通過率、跨角色共用 layer 對位錯誤率、starter 預設外觀重疊率、Yumi 深藍髮與 Mary 深綠髮視覺 QA 通過率、非髮色外觀變動率、SVG／濾鏡／單件 nudge 替代檢出率、頭胸照大頭照與全身著裝衣物對位一致性 QA 通過率（含著裝 bust 服裝錯位至臉部檢出率）、outfit set／outer 雙層殘留檢出率（單品單層不變式，應為 0）、商店預覽與實穿同源（預覽 `image` 即 `layers[0].src`、無分離商品縮圖殘留）檢出率（應為 0）。
...
* **spec#7-可用純靜態網站方式部署並模組化擴充內容**
  * 評估方式：以全新環境依 productReadme 完成部署與內容擴充，並驗證可玩公主、衣物 layer 與場景背景可獨立擴充，且角色、wardrobe 與 sceneArt 素材擴充流程維持 raster 內容包交付、類別級對位組態與 `1024x1024` 場景背景契約可重用。
  * 觀察項目：部署成功率、新增內容包後既有功能未回歸、可玩公主新增／替換成功率、新增 wardrobe item 繼承類別級 layer bounds 成功率、同一衣物 layer 跨角色對位成功率、角色與 wardrobe 素材 raster 交付率、場景背景 raster 交付率、場景背景完整繪製通過率、SVG／濾鏡／模糊補版／renderer 特例替代檢出率。
