...
* **spec#3-可把學習成果轉為看得見的外觀獎勵**
  ...
  * 觀察項目：購買件數、換裝次數、coins 留存、base 對位錯誤率、wardrobe 類別級 layer bounds 覆蓋率、代表性衣物實穿對位 QA 通過率、跨角色共用 layer 對位錯誤率、starter 預設外觀重疊率、Yumi 深藍髮與 Mary 深綠髮視覺 QA 通過率、非髮色外觀變動率、SVG／濾鏡／單件 nudge 替代檢出率、outfit set／outer 雙層殘留檢出率（單品單層不變式，應為 0）。
