...
* **HelmChart參數-values.yaml**
  * [etyCfg通用自架主機平台]（chart values 明確欄位；對照 `sysApi/.env.example` 為組態語意正本）
    ...
    * paramAppHardening=`app 容器 resources 預設（requests cpu 50m／mem 128Mi、limits mem 512Mi，可覆寫）＋securityContext 硬化（runAsNonRoot、不允許提權、drop ALL capabilities、seccomp RuntimeDefault）`（#317）；**db 容器明文不套用**——PostgreSQL 官方 image 啟動時以 root 做 initdb 權限調整後自行降權，容器層強制 runAsNonRoot 會破壞初始化，不做假硬化
  * [etyCfg自訂sysApi組態]
