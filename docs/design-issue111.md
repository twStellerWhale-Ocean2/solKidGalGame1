...
## 5. 3code 實作與驗證結果（CODE-READY，2026-06-14）

* **實作（純 CSS，單一來源收斂）**：
  * [styles/adv.css] `.adv-scene` 新增單一變數 `--adv-scene-height: min(860px, calc(100dvh - 32px))` 並 `height: var(--adv-scene-height)`。
  * [styles/shop.css] 移除 `:is([shop],[wardrobe],[refund])` 外框高度覆寫。
  * [styles/mobile.css] 8 條 `.adv-scene` 外框 height 改為 `var(--adv-scene-height)`、刪除 `[data-mode="shop"]` 高度覆寫；共消除 §1 表列 11 條分歧/重複/逐模式規則。內層版型（grid、立繪、商品格、`--adv-dialog-height`）維持不動。
* **GATE §1（機器判定，全綠）**：`tsc --noEmit` exit 0；`docLint -Level sol` 0（design.md 未動）；`repoLint` 0；`?selftest=monkey`／`save-load` `passed:true`、console 0 error；純靜態無第三方相依（無 audit 對象）。
* **GATE §5（visual-QA，活 build 量測，寬窄兩視口×全模式）**：
  * **外框高度全等**——桌機寬版 1600×900：scene/quest/hint/complete/shop/wardrobe/refund 皆 `860px`；手機窄版 375×812：皆 `780px`。（修前：桌機 quest `680`／shop `860`＝差 180px；手機 quest `760`／shop `720`。）
  * dialog 加高後立繪自適應撐滿（桌機立繪 ~590→`690px`、錨定底部、無底部留白）、無 overflow（scrollH=clientH）。
  * 與 #112（NPC emoji 心情徽章）共存無誤：`.adv-npc::after` 仍正確顯示 `🙂`。
* **並行合併註記**：3code 期間 #112（PR #113）合併入 main，其 commit 夾帶了本案 [styles/adv.css] 的 var 變更（已先落 main）；本案 mobile/shop 收斂與本設計note 隨後補齊並一併整理進 main，使機制完整生效。
* **結論：可宣稱完成（CODE-READY）**。
