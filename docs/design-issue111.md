...
* **D2（目標值・單一響應式公式）**：外框採**單一響應式高度**，於各視口讓最吃高度的 shop 內容容得下、dialog 內容自然撐滿。建議基準 `min(860px, calc(100dvh - 32px))`（即向現行 shop 桌機值對齊），桌機寬版兩模式同為 ~860、手機窄版兩模式同為 ~780。**確切 px/dvh 上限與 dialog 內層自適應，留 3code 跨視口×跨模式 visual-QA 校準**（同 #101 既例）。
...
* **統一高度方向（主決策）**：
  * **方案甲（建議）**：向 **shop 近全高**對齊（~`min(860px, 100dvh-32px)`），兩模式同框。優點：shop 不被擠壓、與 #103 沉浸式覆蓋層方向一致、手機幾乎無變動。代價：桌機 dialog 由 680 升至 ~860，立繪舞台需自適應撐滿（否則底部留白）。
  * **方案乙**：向**較矮（露出更多場景背景）**對齊。優點：呼應原始「露出背景/沉浸感」訴求。代價：壓縮 shop 商品格/試穿預覽、與現行近全高方向相反、改動較大。
* **一致性涵蓋層級**：是否要求**寬版與窄版採同一條響應式公式**（`min()` 自動隨視口縮放，本note 採此），或允許寬窄各定一值但結構一致？（兩者皆達「同視口內 dialog==shop」之核心；建議前者，機制最單純。）
