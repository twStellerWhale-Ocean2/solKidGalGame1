...
* **① design.md 落點 → 初判 Option A（建議，obj 已預核）**：對話方塊配色／透明度／空狀態／角色像素定位皆屬呈現精修，且 design.md 現**未對之設條文**（與既有 portal 形狀、選項配色一致——皆只落在 CSS／renderer）；本案維持同一慣例——design.md ＜I／II＞無 spec# 增刪、決策由本 note 承載、README 補＜變更紀錄＞初稿。
