...
* **工具操作強化（USR 回饋，2026-06-19）**：
  ...
  * ① 類型框（safeBox）定位：為投影區軟性指引＋新素材預設值（既有單品各帶 ② 單品框、不受 ① 即時影響），非逐件控制。
  * **梯形 warp（USR 回饋）**：② 單品框新增 `topInset`／`bottomInset`（canvas px，左右對稱內縮），把矩形以**投影變換（matrix3d）**拉成等腰梯形——上/下邊可不同寬。[game-engine/render/paper-doll.js] 渲染後依 layer 實際尺寸算 matrix3d（加性、未帶 inset 者不變、ResizeObserver 重算）；工具以四角控制點調 inset、overlay 角點移到梯形角、export 帶 inset。
  * **safeBox 退為軟性指引**：手動 per-item 校準可超出類別 safeBox（落在 512×768 畫布內即可）；[game-engine/testing/selftests.js] `data-audit` 對「targetBox 超出 safeBox」由 error 改為 warning、僅「落在畫布外」才 error；[contract-local/hmiIntf自訂角色尺度與美術規範.md] §III.B 同步軟化措辭。
