...
* 3code 已重繪並替換 16 張 `review-soft-band` 場景背景：`urban/garden`、`castle/bedroom`、`castle/castle-seamstress`、`castle/knights-room`、`castle/maids-room`、`castle/royal-cloak-room`、`urban/administration`、`urban/dress-boutique`、`urban/hair-salon`、`urban/harbor`、`urban/library`、`urban/lighthouse`、`urban/market`、`urban/shoes`、`urban/tailor-studio`、`wild/wild-path`。
* 重繪後 browser `?selftest=data-audit` 通過：40 unique runtime scene backgrounds checked、0 errors、0 review-soft-band warnings；contact sheet 40 cards、0 review flags、0 console errors。
