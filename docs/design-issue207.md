...
* **處置（本案，取「反轉預設」而非再加一條 opt-out）**：根因是 `paper-doll.js` 對**每個** surface 無條件吐出 `.avatar-shadow`、再逐 surface 以 `display:none` 關掉（render-then-opt-out）——預設「長圓盤」本身是錯的，將來每新增一個立繪 surface 都會默默繼承不該有的圓盤、且須有人「記得」補關，是會擴大的技術債。故不採「對 `.adv-doll` 再加一條 `display:none`」的 opt-out 補丁，改為**反轉預設為 opt-in**：base `.avatar-shadow { display:none }`，僅 `.map-doll`／`.area-doll` 明確 `display:block` 接地；同時刪除原 `.bust-doll .avatar-shadow { display:none }`（已被新預設涵蓋）。新增 surface 預設即乾淨、無須再記得補關。
* **不變式維持（現有外觀全等）**：① 立繪（`adv`／`bust`／`side`，含 `full-qa`／`outfit-qa`／`tuner`／`bust-qa` 等 QA 變體）原即隱藏 → 改由預設隱藏，視覺不變；② 地圖／地區 token（`map`／`castle-map`／`world-map`／`area-*`）原即顯示 → 改由 opt-in 顯示，視覺不變、地圖 token 分級不動；③ `--character-silhouette-filter` 三層深灰投影與 selftest（`.adv-doll .paper-doll-stage` drop-shadow ≥3）不變；④ `.try-on-active` 暖黃試穿光暈語意分離不受影響。
